import { Col, Row, Typography, Card, FloatButton, Drawer, Form, Input, Button, notification, Popconfirm, Select, Tabs} from "antd";
...
const handleSubmit = () => {
  ...
  // State untuk filter deskripsi
  const [filterDescription, setFilterDescription] = useState("semua");

  // PERBAIKAN: Menggunakan includes() untuk mencari kata berdasarkan filter yang dipilih
  const PlaylistCount = dataSources.filter(
    (item) => {
      if (filterDescription === "semua") return true; // Jika "semua", hitung semua data
  ...
  const filteredData = dataSourceFiltered?.filter((item) => {
    if (activeTab === "semua") return true;
    return item?.play_description?.toLowerCase().includes(activeTab.toLowerCase());
  });
        ...
        <Col xs={23} className="mb-24">
          <Card bordered={false} className="circlebox h-full w-full">
            <div className="header-section">
              <Title level={2} className="gradient-text">Daftar My Playlist</Title>
              <Input
